...
module.exports =
  function st2PacksCtrl($scope, st2api, $filter, Notification, st2FlexTableService) {
    ...
    var savedView = JSON.parse(sessionStorage.getItem('st2PackView'));
    ...
    $scope.$watch('view', function (view) {
      sessionStorage.setItem('st2PackView', JSON.stringify(view));
    }, true);
    ...
    var listUpdate = function () {
      st2api.client.packs.list().then(function (packs) {
        $scope.icons = {};
        $scope.installed = packs;
        console.log(packs);
        console.log(st2api.client.packs);
        ...
        Notification.criticalError(err, 'Failed to update pack list');
      });
      st2api.client.packs.search().then(function (packs) {
        $scope.available = packs;
      }).catch(function (err) {
        Notification.criticalError(err, 'Failed to search the index');
      });
    };
