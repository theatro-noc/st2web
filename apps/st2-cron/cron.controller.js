...
module.exports =
    function st2CronCtrl($scope, st2api, Notification) {
        ...
        $scope.printer = function (toPrint) {
            console.log(toPrint);
        };

        $scope.updateTime = function () {
            $scope.cron.trigger.parameters.date = $scope.toDate($scope.times.theDate)+' '+$scope.toTime($scope.times.theTime);
        };
        ...
        var savedView = JSON.parse(sessionStorage.getItem('st2CronView'));
        ...
        $scope.$watch('view', function (view) {
                sessionStorage.setItem('st2CronView', JSON.stringify(view));
        }, true);
        ...
        $scope.popup = {
            ...
            submit: function () {
                console.log(JSON.stringify($scope.cron));
                if ($scope.cron.trigger.type === 'core.st2.DateTimer'){
                    $scope.cron.trigger.parameters.date = $scope.toDate($scope.times.theDate)+' '+$scope.toTime($scope.times.theTime);
                }

                st2api.client.rules.create(angular.copy($scope.cron)).then(function (rule) {
